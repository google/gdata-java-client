...
$developerKey = "REPLACE WITH YOUR DEVELOPER KEY";
...
function showIntroPage() {
  ...
  print '<tr>' . "\n";
  print '<th colspan="2" style="text-align:center;">PHP Demo: Google Base API</th>' . "\n";
  print '</tr>' . "\n";
...
function showMainMenu($tableTitle, $sessionToken) {
  ...
  print '<tr>' . "\n";
  print '<th colspan="2" style="text-align:center">PHP Demo: Google Base API' . "\n";
  print '</tr>' . "\n";
...
if(count($_GET) == 1 && array_key_exists('token', $_GET)) {
  ...
  if(count($_POST) == 0) {
    ...
    if($_POST['action'] == 'insert') {
      ...
      if(deleteItem()) {
        showMainMenu('Item deleted.', $_POST['token']);
      } else {
        showMainMenu('Item deletion failed.', $_POST['token']);
      }
    } else if($_POST['action'] == 'delete_all') {
      if(batchDelete()) {
        showMainMenu('All items deleted.', $_POST['token']);
      } else {
      ...
      if(updateItem()) {
        showMainMenu('Item successfully updated.', $_POST['token']);
      } else {
        showMainMenu('Item update failed.', $_POST['token']);
      }
    } else {
      showIntroPage();
    }
