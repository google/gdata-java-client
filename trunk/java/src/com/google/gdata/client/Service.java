...
public class Service {
  ...
  /**
   * Returns the {@link MetadataRegistry} that contains the metadata about
   * element types parsed or generated by this service.
   */
  public MetadataRegistry getMetadataRegistry() {
    // subtypes will populate the registry with their types similar to how
    // the extension profile was populated in the previous data model.
  ...
  protected class ClientInputProperties extends ClientStreamProperties
    ...
    protected ClientInputProperties(GDataRequest req, Class<?> expectType)
      ...
      MetadataRegistry metaRegistry = getMetadataRegistry();
      if (Element.class.isAssignableFrom(expectType)) {
        ElementKey<?, ?> key = 
            Element.getDefaultKey(expectType.asSubclass(Element.class));
        elementMetadata = metaRegistry.bind(key, getMetadataContext());
      } else {
  ...
  public class ClientOutputProperties extends ClientStreamProperties
    ...
    protected ClientOutputProperties(GDataRequest req, Object source) {
      ...
      if (source instanceof Element) {
        ...
        elementMetadata = getMetadataRegistry().bind(key, getMetadataContext());
      } else {
        elementMetadata = null;
      }
    }
